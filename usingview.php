...
class PDF_report1 extends TCPDF
{
   ...
   /*     foreach($lines as $line) {
    ...
    public function ColoredTable($header) {
    	...
    	$this->SetFont('', 'B');
        // Header
        $w = array(15,35,10,25,15,20,45,15,30,100,10);//表格欄數為11並設定欄位寬度
        $num_headers = count($header);
        for($i = 0; $i < $num_headers; ++$i) {
        	$this->Cell($w[$i], 4, $header[$i], 'T', 0, 'C', 0);
        }
        $this->Ln();
        $header2 = array('編號', '編號','面積', '姓名', '姓名','關係','啟用日期','種類','電話','地址','備註');
        $num_headers = count($header);
        for($i = 0; $i < $num_headers; ++$i) {
            $this->Cell($w[$i], 4, $header2[$i], 'B', 0, 'C', 0);
        }
        $this->Ln();
        ...
        while($row=mysql_fetch_array($result)){
        	...
        	$this->Cell($w[3], 6, $rightuser, 0, 0, 'C', $fill);
        	$this->Cell($w[4], 6, $username, 0, 0, 'C', $fill);
            $this->Cell($w[5], 6, $relationship, 0, 0, 'C', $fill);
            $this->Cell($w[6], 6, $startday, 0, 0, 'L', $fill);
            $this->cell($w[7] ,6, $type, 0, 0, 'C', $fill);
            $this->cell($w[8] ,6, $phone, 0, 0, 'L', $fill);
            $this->cell($w[9] ,6, $address, 0, 0, 'L', $fill);
        	$this->Cell($w[10], 6, '', 'L', 0, 'R', $fill);
        	$this->Ln();//換行
//            $this->setXY(320.0,0.0);
//            $this->Write(5, substr($base_id,0,3) ."區", '');
    	}
 	}
}
